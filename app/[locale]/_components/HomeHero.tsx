...
// nothing else. The stacked subtree runs to 1439px and the collage engages at
// `desktop:` (1440px) per D-B, so the stacked one is built fluid and carries
// `tokens-mobile-type` to keep the mobile type scale through the 1024–1440 band.
//
...
export default function HomeHero({ locale }: { locale: Locale }) {
  ...
  return (
    <>
      ...
      {/* ---------------------------------------------------------------- */}
      <section className="tokens-mobile-type desktop:hidden">
        ...
        <div className="relative flex items-start justify-center bg-(--color-brand-primary-green) py-[35px]">
          ...
          <div className="flex w-[calc(100%-31px)] max-w-[362px] flex-col items-end gap-[28px]">
            ...
            <div className="flex w-full flex-col items-center gap-[21px]">
              {/* w-full! overrides buttonBaseClassName's lg:w-auto — this is the
                  fill-width mobile layout even when it renders above 1024px. */}
              <Cta href={workHref} tone="dark" className="w-full!">
                {hero.ctaPrimary}
              </Cta>
              {secondaryLink}
            </div>
      ...
      {/* ---------------------------------------------------------------- */}
      {/* Collage — 1440px design (D-B, D-C)                                */}
      {/* ---------------------------------------------------------------- */}
      <section className="hidden bg-(--color-basic-accent) desktop:block">
        <div className="relative mx-auto h-[960px] w-full max-w-[1440px] overflow-hidden">
          ...
          {/* Green panel 12405:6973 — flush to the canvas edge, which is the
              viewport edge at exactly 1440px and inset above it. */}
